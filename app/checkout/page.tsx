...
// Sub-componente que contém toda a lógica da página
function CheckoutContent() {
  ...
  const fetchCEP = async () => {
    ...
    if (cleaned.length !== 8) { setCepStatus("error"); return; }
    setLoadingCep(true);
    setCepStatus("");
    try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const items = cartItems.map((item) => ({
        ...
        price: paymentMethod === "pix" ? (item.pixPrice ?? item.price) : item.price,
      }));
      ...
      <div className="flex-1 min-w-0 space-y-8">
        ...
        <section>
          ...
          <div className="space-y-3">
            <Input placeholder="Nome completo" value={form.name} onChange={(v) => set("name", v)} />
            <Input type="email" placeholder="Email" value={form.email} onChange={(v) => set("email", v)} />
            <Input placeholder="Telefone" value={form.phone} onChange={(v) => set("phone", v)} />
          </div>
        </section>
            ...
            <div>
              <div className="flex gap-2">
                <div className="flex-1">
                  <Input placeholder="CEP" value={form.cep} onChange={handleCepInput} maxLength={9} />
                </div>
                <button
                  onClick={fetchCEP}
                  disabled={loadingCep}
              ...
              {cepStatus === "ok" && (
                <p className="text-[11px] text-green-600 mt-1 pl-1">Endereço encontrado ✓</p>
              )}
              {cepStatus === "error" && (
                <p className="text-[11px] text-red-500 mt-1 pl-1">CEP inválido ou não encontrado</p>
              )}
            </div>

            <div className="flex gap-3">
              <div className="flex-1">
                <Input placeholder="Rua / Av." value={form.street} onChange={(v) => set("street", v)} />
              </div>
              <div style={{ width: "7rem" }}>
                <Input placeholder="Número" value={form.number} onChange={(v) => set("number", v)} />
              </div>
            </div>

            <div className="flex gap-3">
              <div className="flex-1">
                <Input placeholder="Complemento (opcional)" value={form.complement} onChange={(v) => set("complement", v)} />
              </div>
              <div className="flex-1">
                <Input placeholder="Bairro" value={form.neighborhood} onChange={(v) => set("neighborhood", v)} />
              </div>
            </div>

            <div className="flex gap-3">
              <div className="flex-1">
                <Input placeholder="Cidade" value={form.city} onChange={(v) => set("city", v)} />
              </div>
              <div style={{ width: "5rem" }}>
                <Input placeholder="UF" value={form.state} onChange={(v) => set("state", v)} maxLength={2} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            {(["pix", "card"] as const).map((method) => (
              ...
              >
                <span className={`block text-[10px] mb-1 font-semibold tracking-[0.1em] uppercase ${paymentMethod === method ? "text-green-600" : "text-zinc-400"}`}>
                  {method === "pix" ? "Com desconto" : "Parcelável"}
                </span>
                <span className={`text-sm font-semibold ${paymentMethod === method ? "text-green-700" : "text-zinc-600"}`}>
        ...
        <div className="bg-[#FFFFFFFF] border-[1.5px] border-zinc-200 rounded-[22px] p-6 space-y-4">
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex justify-between items-start gap-3 text-sm">
                ...
                <span className="text-zinc-800 font-semibold shrink-0">
                  {formatPrice(
                    (paymentMethod === "pix" ? (item.pixPrice ?? item.price) : item.price) * item.quantity,
                  )}
                </span>
              </div>
            ))}
          ...
          {shipping.price > 0 && (
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-zinc-700 font-medium">{formatPrice(shipping.price)}</span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-[22px] font-bold text-zinc-600">{formatPrice(finalTotal)}</span>
          </div>
...
// Exportação padrão com Suspense
