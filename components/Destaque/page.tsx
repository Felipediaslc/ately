...
export default function FeaturedProducts() {
  const { addToCart } = useCart(); // carrinho
  const { favoriteItems, addFavorite, removeFavorite, isFavorite } = useFavorites(); // favoritos globais
          ...
          {products.map((product) => {
            ...
            return (
              <div key={product.id} className="flex-[0_0_95%] sm:flex-[0_0_90%] md:flex-[0_0_33.33%] lg:flex-[0_0_25%] px-3">
                <div className="bg-white rounded-lg flex flex-col transition hover:shadow-lg">
                  ...
                  <div className="mt-4 space-y-1">
                    ...
                    <p className="text-xs text-gray-400 line-through">{product.pixPrice}</p>

                    <button
                      onClick={() => handleAddToCart(product)}
                      className={`mt-3 w-full py-2 text-sm rounded-md font-medium transition-all ${
                        isAdded ? "bg-green-800 scale-95" : "bg-green-600 hover:bg-green-700 hover:scale-105"
                      } text-white`}
                    >
      ...
      <div className="flex justify-center mt-6 gap-2">
        {scrollSnaps.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${index === selectedIndex ? "bg-gray-800 scale-110" : "bg-gray-400/60"}`}
          />
        ))}
      </div>
    </section>
