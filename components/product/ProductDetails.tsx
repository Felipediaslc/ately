...
export default function ProductDetails({ product }: Props) {
  ...
  if (product.stock === 0) {
    stock = { label: "Esgotado", color: "bg-red-500", text: "text-red-700" };
  } else if (product.stock <= 3) {
    stock = { label: "Últimas unidades", color: "bg-yellow-400", text: "text-yellow-700" };
  } else {
    stock = { label: "Em estoque", color: "bg-green-500", text: "text-green-700" };
  }
      ...
      <div className="lg:w-1/2 flex flex-col">
        ...
        {stock && (
          <div className="flex items-center gap-2 mb-4">
            <span className={`w-2 h-2 rounded-full ${stock.color} ${product.stock > 0 ? "animate-pulse" : ""}`} />
